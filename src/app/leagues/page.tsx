...
function FlagSelect({ value, onChange, options, placeholder, error }: {
  value: string
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      ...
      >
        {selected?.flag && <img src={selected.flag} alt="" style={{ borderRadius: 2, boxShadow: '0 0 0 1px var(--border)', flexShrink: 0 }} />}
        <span style={{ flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{selected?.label ?? placeholder ?? 'Select…'}</span>
        <span style={{ color: 'var(--t4)', fontSize: 10, flexShrink: 0 }}>▾</span>
      </div>
      {open && (
        <div style={{
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {filtered.length === 0 ? (
              ...
              >
                {o.flag && <img src={o.flag} alt="" style={{ borderRadius: 2, boxShadow: '0 0 0 1px var(--border)', flexShrink: 0 }} />}
                <span style={{ fontFamily: 'var(--onest)', fontSize: 13, color: o.value === value ? 'var(--red)' : 'var(--t1)', fontWeight: o.value === value ? 600 : 400 }}>{o.label}</span>
              </div>
            ))}
          </div>
        </div>
